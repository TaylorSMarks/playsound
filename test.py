...
def mockMciSendStringW(command, buf, bufLen, bufStart):
    ...
    if command.startswith(u'close '):
        global sawClose
        sawClose = True
        testCase.assertIn(originalMCISendStringW(command, buf, bufLen, bufStart),
                          [0, 263])  # 263 indicates it's not opened or not recognized. It's fine.
        return 0
    ...
    if command.startswith(u'open ') or command.startswith(u'play '):
        testCase.assertIn(originalMCISendStringW(command, buf, bufLen, bufStart),
                          [0, 306])  # 306 indicates drivers are missing. It's fine.
        return 0
...
class PlaysoundTests(unittest.TestCase):
    ...
    def helper(self, file, approximateDuration, block=True):
        ...
        self.assertTrue(approximateDuration - durationMarginLow <= duration <= approximateDuration + duratingMarginHigh,
                        'File "{}" took an unexpected amount of time: {:.2f} - expected ~{:.2f}'.format(
                            file.encode('utf-8'), duration, approximateDuration))

    testBlockingASCII_MP3 = lambda self: self.helper(u'Damonte.mp3', 1.1)
    testBlockingASCII_WAV = lambda self: self.helper(u'Sound4.wav', 1.3)
    testBlockingCYRIL_WAV = lambda self: self.helper(u'Буква_Я.wav', 1.6)
    testBlockingSPACE_MP3 = lambda self: self.helper(u'Discovery - Go at throttle up (2).mp3', 2.3)
